...
<form  action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
		...
		<div <?php echo (!empty($password_err)) ? 'has-error' : ''; ?>">
			...
			<div>
				<input input type="password" name="password"  value="<?php echo $password; ?>">
				<span><?php echo $password_err; ?></span>
			</div>
		</div>
		<div<?php echo (!empty($repeat_password_err)) ? 'has-error' : ''; ?>">
			<label class="col-sm-3 control-label">Repeat password</label>
			<div class="col-sm-9">
				<input input type="password" name="repeat_password" value="<?php echo $repeat_password; ?>">
		...
		<div<?php echo (!empty($full_name_err)) ? 'has-error' : ''; ?>">
			...
			<div>
				<input input type="text" name="full_name" value="<?php echo $full_name; ?>">
				<span><?php echo $full_name_err; ?></span>
			</div>
		</div>
		<div<?php echo (!empty($street_address_err)) ? 'has-error' : ''; ?>">
			<label>Street address</label>
			<div>
				<input input type="text" name="street" value="<?php echo $street_address; ?>">
				<span><?php echo $street_address_err; ?></span>
			</div>
		</div>
		<div><?php echo (!empty($city_err)) ? 'has-error' : ''; ?>
			<label> City </label>
			<div>
				<input input type="text" name="city" value="<?php echo $city; ?>">
				<span>><?php echo $city_err; ?></span>
			</div>
		</div>
		<div> <?php echo (!empty($zip_err)) ? 'has-error' : ''; ?>
			<label> ZIP </label>
			<div>
				<input input type="text" name="zip" value="<?php echo $zip; ?>">
				<span>><?php echo $zip_err; ?></span>
			</div>
		</div>
	
		<div> <?php echo (!empty($deserve_err)) ? 'has-error' : ''; ?>
			<label>Why do you deserve this job?</label>
				<input input type="text" name="deserve"  value="<?php echo $deserve; ?>">
				<span ><?php echo $deserve_err; ?></span>
		</div>

		<div> <?php echo (!empty($recent_project_err)) ? 'has-error' : ''; ?>
			<label>What was your recent project?</label>
				<input input type="text" name="recent_project" value="<?php echo $recent_project; ?>">
				<span><?php echo $recent_project_err; ?></span>
		</div>
